...
class SessionSummaryDialog(QDialog):
    ...
    def _build_ui(self, s: Dict):
        ...
        self._add_stat(stats, 0, 0, str(s.get("reviews", 0)), "Cartes révisées")
        self._add_stat(stats, 0, 1, f"+{s.get('coins_earned', 0)}", "Pièces gagnées", "#f4a42a")
        self._add_stat(stats, 1, 0, f"+{s.get('xp_earned', 0)}", "XP gagnés", "#66bb6a")
        self._add_stat(stats, 1, 1, f"{s.get('streak', 0)}", "Série", "#ff6600")
